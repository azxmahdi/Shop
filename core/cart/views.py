        ...
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            ...
            if cart.is_product(product_id):
                return JsonResponse(
                    {
                        "status": "ok",
                        "total_payment_product": cart.get_total_product_payment_by_discount(
                            product_id
                        ),
                    }
                )
    ...
    def post(self, request, *args, **kwargs):
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            ...
            try:
                ...
                stock = ProductModel.objects.get(
                ...
                if quantity > stock:
                    ...
                    return JsonResponse(
                        {
                            "status": "error",
                            "message": f" متاسفیم. از این کالا فقط {stock} موجود است ",
                            "selected_quantity": cart.get_product_quantity(
                                product_id
                            ),
                            "total_quantity": cart.get_total_quantity(),
                            "total_payment_product": cart.get_total_product_payment_by_discount(
                ...
                if quantity <= 0:
                    ...
                    return JsonResponse(
                        {
                            "status": "error",
                            "message": f"شما نمی تواند مقدار صفر با منفی وارد کنید",
                            "selected_quantity": cart.get_product_quantity(
                                product_id
                            ),
                            "total_quantity": cart.get_total_quantity(),
                ...
                return JsonResponse(
                    {
                        "status": "success",
                        "message": f"با موفقیت {quantity} از این کالا در سبد خرید شما ثبت شد",
                        "selected_quantity": cart.get_product_quantity(
                            product_id
                        ),
                        "total_quantity": cart.get_total_quantity(),
                ...
                return JsonResponse(
                    {
                        "status": "error",
                        "message": "لطفا یک مقدار درست وارد کنید",
                        "selected_quantity": cart.get_product_quantity(
                            product_id
                        ),
                        "total_quantity": cart.get_total_quantity(),
