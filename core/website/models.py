...
class TeamMembers(models.Model):
    ...
    job_title = models.ManyToManyField(JobTitle, related_name="team_members")
    ...
    def clean(self):
        if not self.profile.user.is_superuser:
            raise ValidationError(
                "Only superusers can create or update an instance."
            )

        if not self.profile.first_name or not self.profile.last_name:
            raise ValidationError(
                "Please complete the profile of the selected user first."
            )

        if (
            not self.pk
            and TeamMembers.objects.filter(profile=self.profile).exists()
        ):
